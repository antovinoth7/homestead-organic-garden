...
/**
 * Single canonical source of the built-in plant dataset.
 *
 * Both `DEFAULT_PLANT_CATALOG` (@/services/plantCatalog) and the default
 * `PlantProfiles` (@/services/plantProfiles) are derived from this object, so the
 * names / varieties / tamilNames / descriptions live in exactly one place.
 */
export interface DefaultPlantCategoryData {
  plants: string[];
...
export const DEFAULT_PLANT_DATA: Record<PlantType, DefaultPlantCategoryData> = {
    vegetable: {
      plants: [
        'Brinjal',
        'Long Brinjal',
