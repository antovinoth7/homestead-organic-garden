...
describe('plannedTypeChange', () => {
  it('moves a garden plant off the dropped category', () => {
    expect(plannedTypeChange('Hibiscus', 'shrub')).toBe('flower');
    expect(plannedTypeChange('Ixora', 'shrub')).toBe('flower');
    expect(plannedTypeChange('Jasmine', 'shrub')).toBe('flower');
    expect(plannedTypeChange('Crossandra', 'shrub')).toBe('flower');
  });

  it('moves the keerai from vegetable to spinach', () => {
    expect(plannedTypeChange('Purslane', 'vegetable')).toBe('spinach');
    expect(plannedTypeChange('Amaranthus', 'vegetable')).toBe('spinach');
    expect(plannedTypeChange('Pasalai Keerai', 'vegetable')).toBe('spinach');
    expect(plannedTypeChange('Fenugreek', 'vegetable')).toBe('spinach');
  });

  it('leaves a plant already on the surviving category alone', () => {
    expect(plannedTypeChange('Hibiscus', 'flower')).toBeNull();
    expect(plannedTypeChange('Purslane', 'spinach')).toBeNull();
  });

  it('leaves every other plant alone', () => {
    expect(plannedTypeChange('Nandiyavattai', 'shrub')).toBeNull();
    expect(plannedTypeChange('Tomato', 'vegetable')).toBeNull();
    expect(plannedTypeChange('', 'shrub')).toBeNull();
    expect(plannedTypeChange(null, null)).toBeNull();
  });
...
describe('planProfileRecategorisation', () => {
  it('returns null when there is nothing to move', () => {
    const profiles = withShrubs({
      Nandiyavattai: makePlantProfile({ plantType: 'shrub', name: 'Nandiyavattai' }),
    });
    expect(planProfileRecategorisation(profiles)).toBeNull();
  });
  ...
  it("carries the user's edits across to the surviving category", () => {
    ...
    const moved = planProfileRecategorisation(profiles);
  ...
  it('is idempotent — a second pass finds nothing left to do', () => {
    ...
    const once = planProfileRecategorisation(profiles);
    expect(once).not.toBeNull();
    expect(planProfileRecategorisation(once as PlantProfiles)).toBeNull();
  });
