import { BED_TYPE_NAME, BED_TYPE_SHORT, bedTypeTitle } from '@/config/beds/bedTypeMeta';
...
describe('bed type metadata', () => {
  it('defines an icon, full name, and short name for every bed type', () => {
    for (const type of ALL_BED_TYPES) {
      expect(BED_TYPE_ICON_KEYS[type]).toBeTruthy();
  ...
  it('has no extra keys beyond the known bed types', () => {
    expect(Object.keys(BED_TYPE_ICON_KEYS).sort()).toEqual([...ALL_BED_TYPES].sort());
