...
import { PLANT_IMAGE_ALIASES, slugifyReferenceKey } from '../../src/config/referenceKeys';
import type { DiseaseEntry, PestEntry } from '../../src/types/database.types';
...
type ManifestKind = 'pest' | 'disease' | 'plant';
...
function buildEntries(): ManifestEntry[] {
  ...
  for (const name of getKnownPlantNames()) {
...
function writePromptsMarkdown(entries: ManifestEntry[]): void {
  const lines: string[] = [
    ...
    'into `assets-src/pests|diseases|plants/`, then run `npm run reference:ingest`.',
    '',
  ];

  for (const kind of ['pest', 'disease', 'plant'] as ManifestKind[]) {
