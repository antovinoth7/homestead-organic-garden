...
- **Built inline, pending extraction into components**: C.7 (FarmHealthCard — has health tiles; needs farm header + capacity bars), C.8 (NeedsAttentionScroll — plant-only; rewire to `alerts.ts`), C.11 (TodayProgressCard — donut+chips done), C.12 (BedsQuickScroll — aggregate card, needs horizontal mini-cards), C.14 (TipStrip — wire to `alerts.ts`).
- **Not yet built (net-new)**: C.1 (PlantNowSection), C.2/C.3 (weather service ✅ / WeatherCard pending), C.4 (AlmanacHighlight + full almanac), C.6 (pest alerts on Today via `alerts.ts`), C.13 (InputReminderStrip), C.15 (on-screen task list).
- **C.9 — re-homed to the Beds tab**: enhance the existing B2.16 `BedRotationView`/`BedListScreen` Rotation tab with season countdown, harvest-gap warnings, full 6-rule checklist, transition-input prescriptions. Home stays a glanceable dashboard (no rotation tab on Home).
