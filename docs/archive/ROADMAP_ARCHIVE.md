# Roadmap Archive — Completed Phases & Shipped Feature Specs

> Moved verbatim from `docs/IMPLEMENTATION_ROADMAP.md` on 2026-07-05 to keep the active
> roadmap small. This file is historical reference only — nothing here is open work.
> Active roadmap: `docs/IMPLEMENTATION_ROADMAP.md`.

---

## Changelog history (older "Previous:" reconciliation notes)

> Previous: June 20, 2026 — **Phase C reconciled + started (dashboard architecture).** Review found much of Phase C was pre-shipped *inline* in `TodayScreen.tsx` (~900 lines) during B2/B3/B4: the progress donut (C.11), garden-health tiles (C.7 partial), needs-attention horizontal scroll (C.8 partial), seasonal tip banner (C.14 partial), bed-overview aggregate (C.12 partial), and season-rhythm + pre-monsoon cards — but built ad-hoc, not on the service/component architecture this roadmap describes. Phase C moved to **🔄 In Progress**. Decision: refactor to the roadmap architecture. **Stage 1 foundations shipped**: `FarmAlert`/`WeatherForecast` types; pure alert aggregation (`services/alertsLogic.ts` + `alerts.ts` re-export, mirroring `beds.ts`↔`bedLogic.ts`); `services/weather.ts` (Open-Meteo, 3h cache); `hooks/useTodayTasks.ts` + `hooks/useWeather.ts`; pure utils `plantHealth.ts`/`taskSummary.ts`/`svgArc.ts`/`plantingNow.ts`; `config/almanac.ts`; tests (31 new, green). Remaining stages: extract inline sections into components (TodayProgressCard/FarmHealthCard/NeedsAttentionScroll/TipStrip/BedsQuickScroll), add net-new (WeatherCard/PlantNowSection/AlmanacHighlight/InputReminderStrip/TaskListSection), recompose `TodayScreen`. **C.9 correction:** the fuller rotation view enhances the existing B2.16 `BedRotationView`/`BedListScreen` Rotation tab — *not* a tab on Home (Home stays a glanceable dashboard). Also: **Phase B2 reconciled against the shipped bed tab** (see `docs/BED_TAB_ROADMAP_ALIGNMENT.md`): B2.11 (SVG diagram) marked superseded by the shipped visualization components (`BedTopDownMap`/`BedLayerStack`/`BedSuccessionTimeline`/`BedRowLayout`/`BedZoneIllustration`); B2.12 tests marked partially done; `BedType`/`BedLayer` and the wizard step list corrected to shipped values (6 steps incl. `BedLayoutStep`); June row-layout / succession-timeline / occupancy work captured in a new B2.17 block; dedicated `BedTasksScreen` removed from the bed tab — bed-level tasks now surface in the Care Plan only (resolves the B2.13 ↔ B2.16 contradiction). Also: **B2.12 closed** (`services/beds.test.ts` + `bedLogic.ts` extraction; backup N/A; emulator CRUD deferred). Also: **B4.5 `SeasonalAdaptationScreen` cut** — re-homed to `TodayScreen` as a Pre-Monsoon Prep card + current-season Care Rhythm card (no standalone screen/route). Also: **fixed pre-existing `growthStage` test flakiness** (`computeExpectedGrowthStage`/`computeAnnualCycleStage` now anchor "now" to noon → stable calendar-day counts; full suite deterministically green). Also: **Phase B — Plants and Phase B4 closed out** — B4.4 Beejamrutha deep-link + InputRecipes More-menu entry; B.3 yield chart + B.6 coconut per-tree (journal-based, `harvestStats.ts`); B.1 botanical section deferred (no data source); roadmap drift reconciled (harvest is journal-based, not a `HarvestLog` collection).
...
- B4.4 Beejamrutha deep-link ✅ (CTA on `PlantDetailScreen` → `InputRecipes#beejamrutha`) + new "Input Recipes" More-menu entry (screen was previously unreachable)
- **Phase B4 complete.** _(B4.5 `SeasonalAdaptationScreen` was cut — content re-homed to TodayScreen cards.)_


---

## Architecture adjustments — shipped (4.1, 4.2)
...
## Completed phases (0, A, A2, B, B2, B3, B4, C, D, E, F)
...
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------ | ------ | ---------------------------------- | ------------------------------ |
| B.1  | F9 form components — EditBotanicalIdentitySection, EditQuickInfoSection, EditRelationshipsSection, EditBeneficialsSection, EditNutritionSection, EditCareGuidanceSection, EditSafetySection                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | M      | Low    | Phase A2 (catalog data)            | ✅ 6/7 (botanical deferred)                    |
| B.2  | Deep-links (2.15) — pest/disease/beneficial chips navigate to reference detail screens                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | S      | Low    | Phase A/A3 + B.1                   | ✅ Done                        |
| B.3  | F5 harvest tracking — **journal-based (no `HarvestLog` collection)**: harvest entries via `JournalEntryType.Harvest` (`harvest_quantity/unit/quality`) + `TaskLog.harvest_weight_kg`; `HarvestHistorySection` (stats + recent list) on `PlantDetailScreen`. **Remaining: yield chart** (`HarvestYieldChart` + pure `harvestStats.ts`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | M      | Medium | Phase 0                            | ✅ Done (journal-based + chart) |
...
| B.6  | Coconut per-tree tracking (2.5) — optional `harvest_tree_number` on `JournalEntry` (not a `HarvestLog`); captured in the harvest form for coconut groves (`record_kind: 'row'`); per-tree breakdown in `HarvestHistorySection` via `groupHarvestsByTree`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | M      | Medium | B.3                                | ✅ Done                 |
...
| B.10 | Tests for new services + components                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | M      | Low    | B.1–B.9                            | 🟡 Pure-logic done                 |
...
- B.1 enriched sections (6 of 7): `EditQuickInfoSection`, `EditNutritionSection`, `EditRelationshipsSection`, `EditCareGuidanceSection`, `EditSafetySection`, `EditBeneficialsSection` (all in `src/components/forms/`) — displayed on `PlantDetailScreen`
- B.1 remaining: `EditBotanicalIdentitySection` ⏭ **Deferred** — the `Edit*Section`s are read-only cards driven by `getPlantCareProfile`, and no botanical data (scientific name / family / origin) exists in profiles/catalog/types; building it would require authoring per-variety botanical data for low marginal value. Revisit if/when botanical data is sourced.
...
| B4.4 | ✅ Deep-link: Beejamrutha CTA on `PlantDetailScreen` (non-coconut) → `InputRecipes` `initialTab: 'beejamrutha'`; **plus** new "Input Recipes" entry in the More menu (`InputRecipesScreen` was previously unreachable) | S      | Low  | B4.3, Phase B2   |
...
- **Post-ship extension (2026-07-05)**: C.3's single `WeatherCard` replaced by a location-aware swipeable deck — `WeatherDeck` + `WeatherPlotCard` (temperature/rain/humidity plots), `useWeatherLocations` (farm + saved districts), `config/zones/districtCoordinates.ts` (per-district coordinates; first step against the hardcoded-Kanyakumari risk). Today dashboard compacted: active-first progress chips with expand toggle (C.11), Garden Health moved above Needs Attention, task list folded into the progress card, vague plant-stress tip dropped.
...
**Post-ship extensions (2026-07-05)**:

- Journal list overhaul: `JournalEntryCard` extracted from `JournalScreen`, swipe edit/delete, compact cards, grid view dropped; delete confirmation moved to the shared `ConfirmDeleteModal` (also adopted by catalog and farm delete flows, replacing `BedDeleteModal`).
- Voice dictation extended beyond the journal content field to all notes/analysis fields app-wide via the reusable `VoiceDictation` component (G10 follow-through) — pest/disease modal, task completion notes, plant edit form, calendar.
- Pest/disease history photos: capture-time compression via `expo-image-manipulator` (`utils/imageCompression.ts`, device-local only per `docs/IMAGE_STORAGE.md`) and a pinch-zoom viewer (`ImageZoomModal` on the shared `usePinchZoom` engine, also used by the journal).
...
- Rotation tab on the **Beds** listing (Phase C.9, reconciled 2026-06-20) — the fuller coordinator view (season countdown, harvest-gap alerts, full 6-rule checklist, transition-input prescriptions, per-bed rotation cards) enhances the existing B2.16 `BedRotationView`/`BedListScreen` Rotation tab. It is **not** a tab on Home — the home dashboard stays a glanceable summary.
...
## Final recommendations — shipped tranches
